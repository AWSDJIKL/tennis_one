...
from hot.common.utils import *
from hot.common.camera import *
from hot.model.mixste.hot_mixste import Model
...
def yolokp_to_hrnetkp(keypoints):
    def turn(y_kp):
        h_kp = np.ones_like(y_kp)
        for h, y, _ in hrnet_yolo_direct_map:
            h_kp[h] = y_kp[y]
        # 裆部 左胯和右胯连线中点
        h_kp[0] = (y_kp[11] + y_kp[12]) / 2
        # 脖子 左肩和右肩连线的中点
        h_kp[8] = (y_kp[5] + y_kp[6]) / 2
        # 身子中部 脖子和裆部连线中点
        h_kp[7] = (h_kp[0] + h_kp[8]) / 2
        # 额头 左眼和右眼连线的中点
        h_kp[10] = (y_kp[1] + y_kp[2]) / 2
        # 嘴巴 额头和脖子连线的中点
        h_kp[9] = (h_kp[8] + h_kp[10]) / 2

        return h_kp

    hrnetkp = []
    for yolokp in keypoints:
        hrnetkp.append(turn(yolokp))
    return hrnetkp
...
def show3Dpose(vals, ax, fix_z):
    ...
    colors = [
        ...
        (0, 1, 0),  # 左手 
        (1, 0, 0),  # 右手
        (float(142 / 255), float(94 / 255), float(46 / 255)),  # 身体
        (1, 0, 1),  # 左腿
        (1, 1, 0)  # 右腿
    ]
...
def get_pose2D(video_path, output_dir):
    ...
    for frame in frame_list:
        cap.write(frame)
    cap.release()
    # keypoints = yolokp_to_hrnetkp(keypoints)
    keypoints = np.expand_dims(np.array(keypoints), 0)
...
def get_pose3D(video_path, output_dir, fix_z):
    ...
    args.previous_dir = 'hot/checkpoint/pretrained/hot_mixste'
    ...
    yolo_kp = np.copy(keypoints)
    h_kp = yolokp_to_hrnetkp(keypoints[0])
    keypoints[0] = h_kp
    print("keypoints shape: {}".format(keypoints.shape))
    cap = cv2.VideoCapture(video_path)
    ...
    with open(os.path.join(output_dir, "angle.txt"), "w") as f:
        for i in tqdm(range(n_chunks)):
            ...
            if pad_left != 0 or pad_right != 0:
                input_2D_no = np.pad(yolo_kp[0][low_index:high_index], ((pad_left, pad_right), (0, 0), (0, 0)),
                                     'edge')
            else:
                input_2D_no = yolo_kp[0][low_index:high_index]
            ...
            for j in range(low_index, high_index):
                ...
                image, angles = show2Dpose(yolo_kp[0][jj], copy.deepcopy(img))
        ...
        for i in tqdm(range(n_chunks)):
            ...
            if pad_left != 0 or pad_right != 0:
                input_2D_no = np.pad(keypoints[0][low_index:high_index], ((pad_left, pad_right), (0, 0), (0, 0)),
                                     'edge')
            else:
    ...
    print('\nGenerating demo...')
    for i in tqdm(range(len(image_2d_dir)), desc="save 2d_pose_img and 3d_pose_img"):
...
def gen_2d_video(video_path, output_dir):
    keypoints = np.load(output_dir + 'input_2D/input_keypoints_2d.npz', allow_pickle=True)['reconstruction']
    cap = cv2.VideoCapture(video_path)
    width = cap.get(cv2.CAP_PROP_FRAME_WIDTH)
    height = cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
