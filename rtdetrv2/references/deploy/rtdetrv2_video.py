...
def load_osnet_model():
    model = torchreid.models.build_model(name='osnet_x1_0', num_classes=1, pretrained=True)
    torchreid.utils.load_pretrained_weights(model, 'osnet_x1_0_imagenet.pth')  # 下载预训练权重
    model.eval()
    return model
...
def draw(images, labels, boxes, scores, video_writer, thrh=0.6):
    font = ImageFont.truetype(font='arial.ttf', size=30)  # 获得字体
    ...
    for j, b in enumerate(box):
        ...
        draw.text((b[0], b[1]), text=f"{class_names[lab[j].item()]} {round(scrs[j].item(),2)}", fill='blue', font=font)
...
def extract_box(frame:Image, labels, boxes, scores, max_size , thrh=0.6):
    ...
    # print("--" * 20)
    # 3.截取box区域，写进视频
    frame = frame.crop((float(box[0]), float(box[1]), float(box[2]), float(box[3])))
    iw, ih = frame.size
...
def main(args,):
    ...
    video_writer = cv2.VideoWriter(os.path.join("video/output", video_name), cv2.VideoWriter_fourcc(*'mp4v'), 30, (w, h))
    crop_image_list = []
    max_size = (0, 0)
    ...
    while True:
        ...
        # print("labels: ", labels)
        # print("boxes: ", boxes)
        draw(frame, labels, boxes, scores, video_writer)
    ...
    crop_video_writer = cv2.VideoWriter(os.path.join("video/output", "crop_" + video_name), cv2.VideoWriter_fourcc(*'mp4v'), 30, max_size)
    ...
    video_path = os.path.join("video/output", "crop_" + video_name)
    frames, keypoints, w, h = get_yolo11_keypoints(video_path)
    output_video = cv2.VideoWriter("video/output/yolo11_crop" + video_name, cv2.VideoWriter_fourcc(*'mp4v'), 30, (w, h))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--video_file', type=str, default='video/input/hikvision_test.mp4')
